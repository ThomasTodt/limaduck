...
void CSVDataset::buildColumns(const std::string& path) {
    ...
    for (int row = 0; row < n; ++row) {
        ...
        for (size_t i = 0; i < realCols.size(); i++) {
            ...
            realData[i][row] = fields[colID].empty() ? std::numeric_limits<float>::quiet_NaN() 
                                                    : std::stof(fields[colID]);
...
std::vector<std::vector<std::string>> CSVDataset::get(int32_t n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!std::getline(this->file_stream, line)) break; 
    ...
    switch (pred->cols->type) {
            ...
            switch (pred->op) {
                case Predicate::Operator::EQ:                        
                    for (int32_t tp : oldTPs) {
                        if (sdx[tpx[tp]] == sdy[tpy[tp]]) newTPs[newLength++] = tp;
                    }                        
                    break;
        ...
        case Column::Type::INTEGER: {
            ...
            switch (pred->op) {
                case Predicate::Operator::EQ:                        
                    for (int32_t tp : oldTPs) {
                        if (idx[tpx[tp]] == idy[tpy[tp]]) newTPs[newLength++] = tp;
                    }                        
                    break;
                case Predicate::Operator::NE:                        
                    for (int32_t tp : oldTPs) {
        ...
        case Column::Type::REAL: {
            ...
            switch (pred->op) {
                case Predicate::Operator::EQ:                        
                    for (int32_t tp : oldTPs) {
                        if (rdx[tpx[tp]] == rdy[tpy[tp]]) newTPs[newLength++] = tp;
                    }                        
                    break;
                case Predicate::Operator::NE:                        
                    for (int32_t tp : oldTPs) {
