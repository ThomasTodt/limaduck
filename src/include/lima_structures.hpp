...
enum class LimaOperator : uint8_t {
    EQ = 0,
    NE = 1,
    GT = 2,
    LE = 3,
...
static std::string LimaOperatorToString(LimaOperator op) {
    switch (op) {
        case LimaOperator::EQ: return "=";
        case LimaOperator::NE: return "!=";
        case LimaOperator::GT: return ">";
        case LimaOperator::LE: return "<=";
        case LimaOperator::LT: return "<";
        case LimaOperator::GE: return ">=";
        default: return "?";
    }
}
...
struct LimaSchema {
    ...
    void Initialize(const std::vector<std::string>& names, const std::vector<LogicalType>& types) {
        ...
        for (idx_t i = 0; i < names.size(); i++) {
            ...
            if (types[i].IsNumeric() || types[i].id() == LogicalTypeId::DATE || types[i].id() == LogicalTypeId::TIMESTAMP) {
                ops = {LimaOperator::EQ, LimaOperator::NE, LimaOperator::GT, LimaOperator::LE, LimaOperator::LT, LimaOperator::GE};
            } else {
...
struct LimaNode {
    ...
    std::string ToString(const LimaSchema& schema) const {
        ...
        bool first = true;
        std::vector<std::string> col_names;
        for (auto &col : schema.columns) col_names.push_back(col.name);
