...
public:
	// Construtor inicial
	PredSet(const std::vector<int32_t> &predProd_ref) : hash_val(0), predProd(predProd_ref) {
	}

	// Construtor de cópia
	PredSet(const PredSet &source) : preds(source.preds), hash_val(source.hash_val), predProd(source.predProd) {
	}
	...
	auto end() const -> decltype(preds.end()) {
		return preds.end();
	}
