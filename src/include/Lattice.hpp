...
public:
	...
	struct Node {
		...
		Edge *getTo(int cp, int p, Lattice<N, E> *parent) {
			return parent->getTo(this, cp, p);
		}
		Edge *fetchTo(int cp, int p, Lattice<N, E> *parent) {
			return parent->fetchTo(this, cp, p);
		}
		Edge *getFrom(int cp, int p, Lattice<N, E> *parent) {
			return parent->getFrom(this, cp, p);
		}
		Edge *fetchFrom(int cp, int p, Lattice<N, E> *parent) {
			return parent->fetchFrom(this, cp, p);
		}
	};
	...
	struct Edge {
		...
		Edge(Node *f, int32_t col, int32_t pred, Node *t) : from(f), to(t), cp(col), p(pred) {
		}
	};
	...
	Lattice(void *base_lat, const std::vector<int32_t> &p_counts) : preds_count(p_counts), base_lattice(base_lat) {
		InitializePredProd();
				...
				if (force_fetch) {
					...
					for (auto &entry : curr->to) {
						for (Edge *e : entry.second) {
							if (e && visited.insert(e->to).second)
								q.push(e->to);
						}
					}
				}
